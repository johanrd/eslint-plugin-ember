...
ruleTester.run('audit:no-invalid-interactive (gts)', rule, {
  valid: [
    ...
    // `<div @click='void 0' role='button' />` translates directly:
    '<template><div role="button" onclick={{this.h}}></div></template>',

    // Native elements don't require role.
    '<template><button onclick={{this.h}}></button></template>',
    '<template><input onclick={{this.h}} /></template>',
    ...
    // --- D1. role="presentation" on a plain element with onClick ---
    // jsx-a11y no-static: VALID (presentation is a "you meant static" opt-out).
    // jsx-a11y no-noninteractive: VALID.
    // Vue: VALID (`<div @click='void 0' role='presentation'/>`).
    // Our rule: INVALID — "presentation" not in INTERACTIVE_ROLES.
    //   '<template><div role="presentation" onclick={{this.h}}></div></template>'
    //   → flagged by us; see invalid section.
    ...
    // Our rule: aria-hidden is not consulted — we still flag. See invalid.
  ...
  invalid: [
    ...
    },
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onkeydown' } }],
    },
    {
      code: '<template><div onkeypress={{this.h}}></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onkeypress' } }],
    },
    {
      code: '<template><div onmousedown={{this.h}}></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onmousedown' } }],
    },
    {
      code: '<template><div onmouseup={{this.h}}></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onmouseup' } }],
    },
    ...
    // Static (no inherent role) elements with onClick — jsx-a11y no-static: invalid; ours: invalid.
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'span', handler: 'onclick' } }],
    },
    {
    ...
    },
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'area', handler: 'onclick' } }],
    },
    {
    ...
    },
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'section', handler: 'onclick' } }],
    },
    {
      code: '<template><object onclick={{this.h}}></object></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'object', handler: 'onclick' } }],
    },
...

    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'main', handler: 'onclick' } }],
    },
    {
      code: '<template><article onclick={{this.h}}></article></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'article', handler: 'onclick' } }],
    },
    {
      code: '<template><aside onclick={{this.h}}></aside></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'aside', handler: 'onclick' } }],
    },
    {
    ...
    },
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'table', handler: 'onclick' } }],
    },
    {
...

    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'a', handler: 'onmousedown' } }],
    },
    {
      code: '<template><span onmousedown={{this.h}}></span></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'span', handler: 'onmousedown' } }],
    },
    {
      code: '<template><section onmousedown={{this.h}}></section></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'section', handler: 'onmousedown' } }],
    },
    ...
    // all in our DISALLOWED_DOM_EVENTS, so parity holds.
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'ondblclick' } }],
    },
    {
      code: '<template><div onmousemove={{this.h}}></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onmousemove' } }],
    },
    {
      code: '<template><div onmouseover={{this.h}}></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onmouseover' } }],
    },
    {
      code: '<template><div onmouseout={{this.h}}></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onmouseout' } }],
    },
    ...
    // D1. role="presentation" on plain element with onClick
    // jsx-a11y (both rules): VALID. Vue: VALID.
    // Ours: `presentation` not in INTERACTIVE_ROLES → flag.
    {
      code: '<template><div role="presentation" onclick={{this.h}}></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onclick' } }],
    },
    {
      code: '<template><div role="presentation" onkeydown={{this.h}}></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onkeydown' } }],
    },

    // D2. aria-hidden should suppress (jsx-a11y + vue)
    // jsx-a11y: `<div onClick aria-hidden />` VALID.
    // Vue:      `<div @click='void 0' aria-hidden='true'/>`  VALID.
    // Ours: aria-hidden is not consulted → flag.
    {
      code: '<template><div onclick={{this.h}} aria-hidden="true"></div></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'div', handler: 'onclick' } }],
    },
    ...
    // ours: not in NATIVE_INTERACTIVE_ELEMENTS → flag.
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'menuitem', handler: 'onclick' } }],
    },
    {
      code: '<template><datalist onclick={{this.h}}></datalist></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'datalist', handler: 'onclick' } }],
    },
    {
      code: '<template><option onclick={{this.h}} class="foo"></option></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'option', handler: 'onclick' } }],
    },
    ...
    // `controls` — an element without user-facing affordances is not interactive.)
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'audio', handler: 'onclick' } }],
    },
    {
      code: '<template><video onclick={{this.h}}></video></template>',
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'video', handler: 'onclick' } }],
    },
    ...
    // Ours: explicitly excludes hidden input from native-interactive → flag.
    {
      ...
      output: null,
      errors: [{ messageId: 'noInvalidInteractive', data: { tagName: 'input', handler: 'onclick' } }],
    },
