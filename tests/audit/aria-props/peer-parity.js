...
ruleTester.run('audit:aria-props (gts)', rule, {
  valid: [
    ...
    // === DIVERGENCE — Angular: `<app-custom aria-x="text">` is VALID ===
    // Angular skips custom components (tags with a hyphen). Our rule does not
    // distinguish element types — it flags `aria-x` on any tag. See invalid
    // section.

    // === DIVERGENCE — Angular: `<app-test aria-expanded="notABoolean">` VALID ===
    // Angular skips value validation on custom components. Our rule flags on
    // any tag. See invalid section.
  ],
  ...
  invalid: [
    ...
    // === DIVERGENCE — Angular: `<app-custom aria-x="text">` is VALID for Angular ===
    // Angular skips aria validation on custom components. Our rule does not
    // distinguish; it flags on any tag that has an aria-* attribute.
    // This captures our stricter behavior.
    {
      code: '<template><app-custom aria-x="text">Text</app-custom></template>',
      output: null,
      errors: [{ messageId: 'noInvalidAriaAttribute', data: { attribute: 'aria-x' } }],
    },

    // === DIVERGENCE — Angular: `<app-test aria-expanded="notABoolean">` VALID ===
    // Same reason. We flag the value because our rule doesn't special-case
    // custom components.
    {
      code: '<template><app-test aria-expanded="notABoolean"></app-test></template>',
      output: null,
      errors: [
        {
          messageId: 'invalidAriaAttributeValue',
...
hbsRuleTester.run('audit:aria-props (hbs)', rule, {
  ...
  invalid: [
    ...
    // DIVERGENCE — Angular flags `<app-custom aria-x="text">` as VALID
    // (custom-component skip). We flag.
    {
      code: '<app-custom aria-x="text">Text</app-custom>',
      output: null,
      errors: [{ messageId: 'noInvalidAriaAttribute', data: { attribute: 'aria-x' } }],
    },
  ],
