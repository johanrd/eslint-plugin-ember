...
ruleTester.run('audit:role-supports-aria-props (gts)', rule, {
  ...
  invalid: [
    ...
    // === DIVERGENCE — <input type="email"> implicit role ===
    // jsx-a11y considers these to be "textbox" (since aria-query's first
    //   "email" entry has "list attribute not set" constraint → textbox).
    // Our rule iterates elementRoles.keys() and returns the FIRST entry for
    //   type=email, which is "combobox".
    // aria-level unsupported in either; still flagged. Message differs.
    {
