...
function buildSubmitLikeInputTypes() {
  ...
  for (const [schema, rolesSet] of elementRoles) {
    if (schema.name !== 'input') continue;
    if (![...rolesSet].includes('button')) continue;
    const typeAttr = (schema.attributes || []).find((a) => a.name === 'type');
    if (!typeAttr || typeof typeAttr.value !== 'string') continue;
    if (NON_SUBMITTING_TYPES.has(typeAttr.value)) continue;
...
function getShareCategory(tag, type) {
  ...
  if (tag === 'button') {
    ...
    if (type === 'submit') return 'submit-like';
    return null;
  }
  if (tag === 'input' && SUBMIT_LIKE_INPUT_TYPES.has(type)) return 'submit-like';
  return null;
}
