  ...
  create(context) {
    ...
    function checkCoRenderingCluster(entries) {
      ...
      const coRendering = entries.filter((entry) =>
        entries.some(
          (other) => other !== entry && !areMutuallyExclusive(entry.path, other.path)
        )
      );
    ...
    return {
      ...
      'Program:exit'() {
        // Group by (scopeRoot, role); each cluster independently checked.
        const clusters = new Map();
        for (const entry of landmarks) {
          const key = `${
            entry.scopeRoot ? `${entry.scopeRoot.loc?.start?.line || 0}:${entry.scopeRoot.loc?.start?.column || 0}` : 'root'
          }::${entry.role}`;
          if (!clusters.has(key)) {
            clusters.set(key, []);
          }
