...
// but NOT whether a field name's control group matches the input type (e.g.
// `current-password` on `type="text"`). That cross-check is in html-validate
// but is NOT in axe-core's `autocomplete-valid` — the a11y reference used by
// jsx-a11y and lit-a11y (both delegate to axe-core's virtual rule). We side
// with axe-core: the HTML autofill mapping has enough real-world variance
// that this is in the "when in doubt, don't flag" zone.
