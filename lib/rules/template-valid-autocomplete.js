...
// Enforce multiplicity/mutual-exclusion rules for non-field token classes.
// Returns `{ messageId, data }` on the first violation found, or `null` if the
// token list satisfies all multiplicity constraints.
function checkMultiplicity(order) {
  const counts = { section: 0, hint: 0, webauthn: 0 };
  const contactCounts = new Map();
  for (const { tok, kind } of order) {
    if (kind === 'section' || kind === 'hint' || kind === 'webauthn') {
      counts[kind]++;
    } else if (kind === 'contact') {
      contactCounts.set(tok, (contactCounts.get(tok) ?? 0) + 1);
    }
  }
  if (counts.section > 1) {
  ...
  for (const [tok, count] of contactCounts) {
    if (count > 1) {
      return { messageId: 'duplicateContact', data: { token: tok } };
    }
  }
...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      duplicateContact: '`"{{token}}"` may appear at most once in autocomplete',
