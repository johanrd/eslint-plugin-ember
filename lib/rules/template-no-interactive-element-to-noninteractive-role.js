...
function attrConstraintMatches(baseAttr, node) {
  ...
  if (constraints.includes('undefined')) {
    ...
    // Attribute is present. 'undefined' means "this schema entry requires the
    // attribute to be absent". Aria-query currently never pairs 'undefined'
    // with a 'value' on the same attribute (verified against elementRoles), so
    // attr-present is always a non-match here. Explicit early-return removes
    // the previous ambiguous fall-through to `return Boolean(attr)`, which
    // silently mis-matched schemas like `<select multiple>` against the
    // combobox entry (aria-query says `multiple` promotes it to listbox).
    return false;
  }
