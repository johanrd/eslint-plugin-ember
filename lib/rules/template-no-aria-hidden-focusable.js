...
// whitespace-trimmed, case-insensitive. ARIA allows `true`/`false`/missing;
// any other value is treated as the default (false) by UAs.
...
function isAriaHiddenTruthy(node) {
  ...
  if (value.type === 'GlimmerTextNode') {
    const chars = normalizeToken(value.chars);
  ...
  if (value.type === 'GlimmerConcatStatement') {
    ...
    if (parts.length === 1) {
      ...
      if (only.type === 'GlimmerTextNode') {
        const chars = normalizeToken(only.chars);
        return chars === '' || chars === 'true';
      }
    }
    return false;
