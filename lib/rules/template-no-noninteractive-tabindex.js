...
function getStaticTabindexValue(attr) {
  ...
  if (value.type === 'GlimmerTextNode') {
    const stripped = value.chars.trim();
    if (!/^[+-]?\d+$/.test(stripped)) {
      return undefined;
    }
  ...
  if (value.type === 'GlimmerMustacheStatement' && value.path) {
    ...
    if (value.path.type === 'GlimmerStringLiteral') {
      const stripped = value.path.value.trim();
      if (!/^[+-]?\d+$/.test(stripped)) {
        return undefined;
      }
      return Number.parseInt(stripped, 10);
  ...
  create(context) {
    ...
    return {
      GlimmerElementNode(node) {
        ...
        // Skip when tabindex won't actually render. Bare `{{false}}` / `{{null}}`
        // / `{{undefined}}` cause Glimmer to omit the attribute (doc rows t6,
        // t7); dynamic mustache values are unknown at lint time. See
        // docs/glimmer-attribute-behavior.md.
        if (classifyAttribute(tabindex).presence !== 'present') {
          return;
        }
