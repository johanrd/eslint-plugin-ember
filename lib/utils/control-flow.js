 ...
 * actually co-exist at runtime. The only Handlebars/Glimmer construct that
 * introduces mutual exclusion is `{{#if}}` / `{{#unless}}`: the `program`
 * branch renders when the condition is truthy (for `if`) or falsy (for
 * `unless`), and the `inverse` branch renders otherwise. Two elements living
 * in different branches of the SAME if/unless never both render.
 *
 * Other block helpers (`{{#each}}`, `{{#let}}`, `{{#with}}`) do NOT introduce
 * mutual exclusion:
 *   - `{{#each}}` iterates: its body renders 0..N times, but every iteration
 *     also renders whatever sits outside the block. Multiple static children
 *     in the body are rendered N times each — still duplicates at runtime.
 *   - `{{#let}}` / `{{#with}}` are pure scope-binding: the body renders
 *     unconditionally.
 *
 * So these helpers treat if/unless specially and leave other blocks alone.
 *
...
/**
 * Compute the if/unless branch path from the root down to `node`. Each entry
 ...
 *
 * Returns `[]` for elements not inside any if/unless.
 *
...
/**
 * Two paths are mutually exclusive if they share an ancestor if/unless but
 * diverge on its `program` vs `inverse` branch. Elements with mutually-
 ...
 *     if (isIfOrUnlessBlockStatement(node)) scope.enterConditional();
 *   },
 *   'GlimmerBlockStatement:exit': (node) => {
 *     if (isIfOrUnlessBlockStatement(node)) scope.exitConditional();
 *   },
 *   'GlimmerBlock': (node) => {
 *     if (isIfOrUnlessBlockBranch(node)) scope.enterConditionalBranch();
 *   },
 *   'GlimmerBlock:exit': (node) => {
 *     if (isIfOrUnlessBlockBranch(node)) scope.exitConditionalBranch();
 *   },
 *
